...
/**
 * Parses an OSM file (xml, zipped xml or pbf) and creates a graph from it. The
 * OSM file is actually read twice.
 * During the first scan we determine the 'type' of each node, i.e. we check
 * whether a node only appears in a single way
 * or represents an intersection of multiple ways, or connects two ways. We also
 * scan the relations and store them for
 * each way ID in memory.
 * During the second scan we store the coordinates of the nodes that belong to
 * ways in memory and then split each way
 * into several segments that are divided by intersections or barrier nodes.
 * Each segment is added as an edge of the
 * resulting graph. Afterwards we scan the relations again to determine turn
 * restrictions.
 **/
public class OSMReader {
    ...
    public OSMReader(BaseGraph baseGraph, OSMParsers osmParsers, OSMReaderConfig config) {
        ...
        this.restrictionSetter = new RestrictionSetter(baseGraph, osmParsers.getRestrictionTagParsers().stream()
                .map(RestrictionTagParser::getTurnRestrictionEnc).toList());
    ...
    /**
     * Sets the OSM file to be read. Supported formats include .osm.xml, .osm.gz and
     * .xml.pbf
     */
    ...
    /**
     * The area index is queried for each OSM way and the associated areas are added
     * to the way's tags
     */
     ...
     * {@link WaySegmentParser}. All OSM
     * ways that are not accepted here and all nodes that are not referenced by any
     * such way will be ignored.
     */
    ...
    /**
     * @return true if the given node should be duplicated to create an artificial
     *         edge. If the node turns out to be a
     *         junction between different ways this will be ignored and no
     *         artificial edge will be created.
     */
    ...
    /**
     * @return true if the length of the way shall be calculated and added as an
     *         artificial way tag
     */
    ...
    /**
     * This method is called during the second pass of {@link WaySegmentParser} and
     * provides an entry point to enrich
     * the given OSM way with additional tags before it is passed on to the tag
     * parsers.
     */
    protected void setArtificialWayTags(PointList pointList, ReaderWay way, double distance,
        ...
        // we have to remove existing artificial tags, because we modify the way even
        // though there can be multiple edges
        // per way. sooner or later we should separate the artificial ('edge') tags from
        // the way, see discussion here:
        ...
        if (areaIndex != null) {
            ...
            if (pointList.size() > 2) {
                ...
                double lastLat = pointList.getLat(pointList.size() - 1),
                        lastLon = pointList.getLon(pointList.size() - 1);
        ...
        // special handling for countries: since they are built-in with GraphHopper they
        // are always fed to the EncodingManager
        ...
        for (CustomArea customArea : customAreas) {
            ...
            // the country string must be either something like US-CA (including
            // subdivision) or just DE
            ...
            if (
            // countries with subdivision overrule those without subdivision as well as
            // bigger ones with subdivision
    ...
    /**
     * This method is called for each segment an OSM way is split into during the
     * second pass of {@link WaySegmentParser}.
     *
     ...
     * @param nodeTags  node tags of this segment. there is one map of tags for each
     *                  point.
     */
    protected void addEdge(int fromIndex, int toIndex, PointList pointList, ReaderWay way,
        ...
        if (pointList.size() != nodeTags.size())
            throw new AssertionError("there should be as many maps of node tags as there are points. node tags: "
                    + nodeTags.size() + ", points: " + pointList.size());

        // todo: in principle it should be possible to delay elevation calculation so we
        // do not need to store
        // elevations during import (saves memory in pillar info during import). also
        // note that we already need to
        // to do some kind of elevation processing (bridge+tunnel interpolation in
        // GraphHopper class, maybe this can
        // go together
        ...
        if (pointList.is3D()) {
            ...
            // smooth the elevation before calculating the distance because the distance
            // will be incorrect if calculated afterwards
            if (config.getElevationSmoothing().equals("ramer"))
                ...
                throw new AssertionError(
                        "Unsupported elevation smoothing algorithm: '" + config.getElevationSmoothing() + "'");
        }
        ...
        if (distance < 0.001) {
            // As investigation shows often two paths should have crossed via one identical
            // point
        ...
        if (Double.isNaN(distance)) {
            LOGGER.warn("Bug in OSM or GraphHopper (" + bugCounter++ + "). Illegal tower node distance " + distance
                    + " reset to 1m, osm way " + way.getId());
            distance = 1;
        }

        if (Double.isInfinite(distance) || distance > maxDistance) {
            // Too large is very rare and often the wrong tagging. See #435
            // so we can avoid the complexity of splitting the way for now (new towernodes
            // would be required, splitting up geometry etc)
            // For example this happens here: https://www.openstreetmap.org/way/672506453
            // (Cape Town - Tristan da Cunha ferry)
            LOGGER.warn("Bug in OSM or GraphHopper (" + bugCounter++ + "). Too big tower node distance " + distance
                    + " reset to large value, osm way " + way.getId());
        ...
        // If the entire way is just the first and last point, do not waste space
        // storing an empty way geometry
        if (pointList.size() > 2) {
            // the geometry consists only of pillar nodes, but we check that the first and
            // last points of the pointList
    ...
    private void checkCoordinates(int nodeIndex, GHPoint point) {
        ...
        if (Math.abs(nodeAccess.getLat(nodeIndex) - point.getLat()) > tolerance
                || Math.abs(nodeAccess.getLon(nodeIndex) - point.getLon()) > tolerance)
            throw new IllegalStateException("Suspicious coordinates for node " + nodeIndex + ": ("
                    + nodeAccess.getLat(nodeIndex) + "," + nodeAccess.getLon(nodeIndex) + ") vs. (" + point + ")");
    }
    ...
    private void checkDistance(EdgeIteratorState edge) {
        ...
        if (Double.isInfinite(edgeDistance))
            throw new IllegalStateException(
                    "Infinite edge distance should never occur, as we are supposed to limit each distance to the maximum distance we can store, #435");
            ...
            throw new IllegalStateException(
                    "Suspicious distance for edge: " + edge + " " + edgeDistance + " vs. " + geometryDistance
                            + ", difference: " + (edgeDistance - geometryDistance));
    }

    /**
     * This method is called for each way during the second pass and before the way
     * is split into edges.
     * We currently use it to parse road names and calculate the distance of a way
     * to determine the speed based on
     * the duration tag when it is present. The latter cannot be done on a per-edge
     * basis, because the duration tag
    ...
    protected void preprocessWay(ReaderWay way, WaySegmentParser.CoordinateSupplier coordinateSupplier,
        ...
        if (config.isParseWayNames()) {
            ...
            if (way.hasTag("destination:ref")) {
                ...
                if (!fwdStr.isEmpty() || !bwdStr.isEmpty())
                    map.put(STREET_DESTINATION_REF,
                            new KValue(fwdStr.isEmpty() ? null : fwdStr, bwdStr.isEmpty() ? null : bwdStr));
            }
        ...
        if (way.getTags().size() > 1) // at least highway tag
            for (Map.Entry<String, Object> entry : way.getTags().entrySet()) {
                ...
                // for now reduce index size a bit and focus on access tags
                    ...
                    String value = KVStorage
                            .cutString(((String) entry.getValue()).replace(" ", "").replace("bicycle", "bike"));
        ...
        if (Double.isNaN(distance)) {
            // Some nodes were missing, and we cannot determine the distance. This can
            // happen when ways are only
            // included partially in an OSM extract. In this case we cannot calculate the
            // speed either, so we return.
            LOGGER.warn("Could not determine distance for OSM way: " + way.getId());
            return;
        }
        way.setTag("way_distance", distance);

        // For ways with a duration tag we determine the average speed. This is needed
        // for e.g. ferry routes, because
        // the duration tag is only valid for the entire way, and it would be wrong to
        // use it after splitting the way
        // into edges.
        String durationTag = way.getTag("duration");
        if (durationTag == null) {
            // no duration tag -> we cannot derive speed. happens very frequently for short
            // ferries, but also for some long ones, see: #2532
            if (isFerry(way) && distance > 500_000)
                OSM_WARNING_LOGGER.warn("Long ferry OSM way without duration tag: " + way.getId() + ", distance: "
        ...
        if (speedInKmPerHour < 0.1d) {
            // Often there are mapping errors like duration=30:00 (30h) instead of
            // duration=00:30 (30min). In this case we
            // ignore the duration tag. If no such cases show up anymore, because they were
            // fixed, maybe raise the limit to find some more.
            OSM_WARNING_LOGGER.warn(
                    "Unrealistic low speed calculated from duration. Maybe the duration is too long, or it is applied to a way that only represents a part of the connection? OSM way: "
                            + way.getId() + ". duration=" + durationTag + " (= " + Math.round(durationInSeconds / 60.0)
                            +
                            " minutes), distance=" + distance + " m");
            return;
        }
        // tag will be present if 1) isCalculateWayDistance was true for this way, 2) no
        // OSM nodes were missing
        // such that the distance could actually be calculated, 3) there was a duration
        // tag we could parse, and 4) the
    ...
    private double calcDistance(ReaderWay way, WaySegmentParser.CoordinateSupplier coordinateSupplier) {
        ...
        for (int i = 1; i < nodes.size(); i++) {
            ...
            if (Double.isNaN(point.ele) == is3D)
                throw new IllegalStateException(
                        "There should be elevation data for either all points or no points at all. OSM way: "
                                + way.getId());
            distance += is3D
    ...
    protected void preprocessRelations(ReaderRelation relation) {
        if (!relation.isMetaRelation() && relation.hasTag("type", "route")) {
            // we keep track of all route relations, so they are available when we create
            // edges later
    ...
    private void addRestrictionsToGraph() {
        ...
        // The OSM restriction format is explained here:
        // https://wiki.openstreetmap.org/wiki/Relation:restriction
        List<Triple<ReaderRelation, RestrictionTopology, RestrictionMembers>> restrictionRelationsWithTopology = new ArrayList<>(
                restrictionRelations.size());
        for (ReaderRelation restrictionRelation : restrictionRelations) {
            try {
                // Build the topology of the OSM relation in the graph representation. This only
                // needs to be done once for all
                // vehicle types (we also want to print warnings only once)
                restrictionRelationsWithTopology.add(OSMRestrictionConverter.buildRestrictionTopologyForGraph(baseGraph,
                        restrictionRelation, restrictedWaysToEdgesMap::getEdges));
            } catch (OSMRestrictionException e) {
                warnOfRestriction(restrictionRelation, e);
            }
        }
        // It is important to set the restrictions for all parsers/encoded values at
        // once to make
        // sure the resulting turn restrictions do not interfere.
        List<RestrictionSetter.Restriction> restrictions = new ArrayList<>();
        // For every restriction we set flags that indicate the validity for the
        // different parsers
            ...
            try {
                ...
                for (int i = 0; i < osmParsers.getRestrictionTagParsers().size(); i++) {
                    ...
                    if (restrictionType != null && res.getRestrictionType() != restrictionType)
                        // so far we restrict ourselves to restriction relations that use the same type
                        // for all vehicles
                ...
                if (bits.cardinality() > 0) {
                    List<RestrictionSetter.Restriction> tmpRestrictions = OSMRestrictionConverter
                            .buildRestrictionsForOSMRestriction(baseGraph, r.second, restrictionType);
    ...
    private static void warnOfRestriction(ReaderRelation restrictionRelation, OSMRestrictionException e) {
        ...
        if (!e.isWithoutWarning()) {
            ...
            List<String> members = restrictionRelation.getMembers().stream()
                    .map(m -> m.getRole() + " " + m.getType().toString().toLowerCase() + " " + m.getRef())
                    .collect(Collectors.toList());
            OSM_WARNING_LOGGER.warn("Restriction relation " + restrictionRelation.getId() + " " + e.getMessage()
                    + ". tags: " + restrictionRelation.getTags() + ", members: " + members + ". Relation ignored.");
        }
    }
